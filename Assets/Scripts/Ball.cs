...
public class Ball : MonoBehaviour
{
    ...
    [SerializeField] private float velocityMultiplier = 1.1f, velocidadMaxima = 5f;
    ...
    [SerializeField] private GameObject powerUpPrefab, powerDownPrefab, doublePrefab;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("block")) // Si colisionamos con un bloque
        {
            ...
            if (!(Mathf.Abs(rb.velocity.x) > velocidadMaxima)) {
            ...
            // Quitar Vida
            if (random > 0.1f && random < 0.2f)
            {
                Instantiate(powerDownPrefab, collision.transform.position, Quaternion.identity);
            }
            // Agrandar
            if (random > 0.2f && random < 0.35f)
            {
                Instantiate(doublePrefab, collision.transform.position, Quaternion.identity);
            }
